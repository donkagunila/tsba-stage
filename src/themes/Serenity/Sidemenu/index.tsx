    ...
    return <div>
        <div className="flex overflow-hidden">
            ...
            <div className="fixed top-0 w-[85px] xl:w-[360px] overflow-x-hidden z-50 h-screen flex bg-primary">
                <div className="w-[75px] h-full border-r border-white/10 py-16 flex justify-between items-center flex-col">
                    <div>
                        <ul className={"flex gap-2 w-full flex-col justify-center items-center"}>
                            {menus.map((item: SideMenuItem,  index: number) => (
                                <SideMenuItem key={index} item={item} selectedMenu={selectedMenu ?? ""} onSelect={setSelectedMenu} />
                            ))}
                        </ul>
                    </div>
