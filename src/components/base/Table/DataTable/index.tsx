...
const DataTable = (props: Props) => {
    ...
    const renderHeaders = () => {
        return (
            <tr className="text-left text-xs py-4 bg-slate-100/30 dark:bg-darkmode-600 border-y border-slate-200 text-red-600 dark:text-slate-200 capitalize  ">
                {hasSelect && (
                    <th key="checkbox" scope="col" className={"px-2 py-3 capitalize flex-1"}>
                        <input
                            type="checkbox"
                            className={twMerge(
                ...
                {columns.map((column) => (
                    <th
                        key={column.name}
                        scope="col"
                        className="px-2 py-3 capitalize cursor-pointer flex-1"
                        onClick={() => toggleSorting(column.field)}
                    >
                        <div className="flex min-w-[60px]">
                            <span className="text-cyan-700/50 text-sm"> {column.name}</span>
                ...
                >
                    ...
                    {columns.map((column) => {
                        ...
                        return (
                            <td
                                key={column.field}
                                className="px-2 py-4 text-xs text-slate-500"
                            >
                                {column.type === "date"
                ...
                <div className="flex items-center justify-between p-3">
                    ...
                    <div className="mx-1 flex items-center">
                        <FormInput
                            ...
                            className="min-h-[36px]  min-w-[260px] shadow-sm border-slate-200"
                            value={searchQuery}
                ...
                <Lucide name="Printer" className="w-4 h-4 mr-2" /> Print
                            ...
                            <Menu className="w-1/2 sm:w-auto">
                                {!hideExportButton && <Menu.Button
                                    as={Button}
                                    variant="outline-secondary"
                                    className="border-slate-300 text-slate-500 w-full sm:w-auto text-xs min-h-[36px]"
                                >
                                    <Lucide icon="FileText" className="w-3 h-4 mr-2"/> Export
                                    <Lucide
                                        icon="ChevronDown"
                        ...
                        {showCreateButton && (
                            <Button
                                ...
                                className="px-4 py-1.5 min-h-[36px] text-xs min-w-[60px] ml-2"
                                onClick={() => {
